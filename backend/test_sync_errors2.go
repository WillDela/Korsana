...
func main() {
	...
	err = db.Get(&accessToken, "SELECT access_token FROM connected_integrations LIMIT 1")
	if err != nil {
	...
	err = db.Get(&userID, "SELECT user_id FROM connected_integrations LIMIT 1")
	if err != nil {
	...
	fmt.Printf("Successfully inserted %d activities\n", successCount)
}
