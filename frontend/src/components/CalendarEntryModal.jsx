...
const CalendarEntryModal = ({ isOpen, onClose, onSave, onDelete, entry, selectedDate }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      // Parse pace mm:ss/mi to seconds/km
      let pacePerKm = null;
      if (targetPace) {
        ...
        if (parts.length === 2) {
          ...
          if (!isNaN(mins) && !isNaN(secs)) {
            pacePerKm = Math.round((mins * 60 + secs) / 1.60934);
          }
        }
      }
            ...
            <div style={{ padding: '1.5rem' }}>
              ...
              {isEditing && entry?.source && entry.source !== 'manual' && (
                <div style={{
                  ...
                  }}>{entry.source === 'ai_coach' ? 'AI' : entry.source}</span>
                  <span style={{ fontSize: '0.75rem', color: '#5A6178' }}>
                    Generated by {entry.source === 'ai_coach' ? 'AI Coach' : entry.source}
                  </span>
                </div>
              )}
